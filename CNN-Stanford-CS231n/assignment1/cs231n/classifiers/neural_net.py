...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # shape (N, C).                                                             #
    #############################################################################
    hidden_scores = np.maximum(np.dot(X,W1) + b1,0)
    ...
    loss = -np.sum(keepProb * np.log(prob)) / num_train + 0.5 * reg * (np.sum(W1*W1)+np.sum(W2*W2))
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      choice_data = np.random.choice(num_train,batch_size,replace=False)
      X_batch = X[choice_data]
      y_batch = y[choice_data]
