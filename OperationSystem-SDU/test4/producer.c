...
int main(int argc, char *argv[]) {
	...
	buff_ptr = (char *) set_shm(buff_key, buff_num, shm_flg); // 函数建立一个具有n个字节的共享内存区
	//获取生产者放产品位置指针 pput_ptr 指向缓冲区首地址
	pput_ptr = (int *) set_shm(pput_key, pput_num, shm_flg);// 函数建立一个具有n个字节的共享内存区
	//信号量使用的变量
	...
	prod_sem = set_sem(prod_key, sem_val, sem_flg); //建立一个具有 n 个信号灯的信号量
	//消费者初始无产品可取,同步信号灯初值设为 0
	sem_val = 0;
	//获取消费者同步信号灯,引用标识存 cons_sem
	cons_sem = set_sem(cons_key, sem_val, sem_flg);	//建立一个具有 n 个信号灯的信号量
	//生产者互斥信号灯初值为 1
	sem_val = 1;
	//获取生产者互斥信号灯,引用标识存 pmtx_sem
	pmtx_sem = set_sem(pmtx_key, sem_val, sem_flg);	//建立一个具有 n 个信号灯的信号量
	//循环执行模拟生产者不断放产品
	while (1) {
		//如果缓冲区满则生产者阻塞
		down(prod_sem);
